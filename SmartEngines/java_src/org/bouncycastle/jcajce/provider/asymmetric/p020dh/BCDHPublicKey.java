...
public class BCDHPublicKey implements DHPublicKey {
    ...
    /* renamed from: y */
    private BigInteger f1563y;

    BCDHPublicKey(BigInteger bigInteger, DHParameterSpec dHParameterSpec) {
        this.f1563y = bigInteger;
    ...
    BCDHPublicKey(DHPublicKey dHPublicKey) {
        this.f1563y = dHPublicKey.getY();
        this.dhSpec = dHPublicKey.getParams();
        this.dhPublicKey = new DHPublicKeyParameters(this.f1563y, new DHParameters(this.dhSpec.getP(), this.dhSpec.getG()));
    }

    BCDHPublicKey(DHPublicKeySpec dHPublicKeySpec) {
        this.f1563y = dHPublicKeySpec.getY();
        this.dhSpec = new DHParameterSpec(dHPublicKeySpec.getP(), dHPublicKeySpec.getG());
        this.dhPublicKey = new DHPublicKeyParameters(this.f1563y, new DHParameters(dHPublicKeySpec.getP(), dHPublicKeySpec.getG()));
    }

    public BCDHPublicKey(SubjectPublicKeyInfo subjectPublicKeyInfo) {
        this.info = subjectPublicKeyInfo;
        try {
            this.f1563y = ((ASN1Integer) subjectPublicKeyInfo.parsePublicKey()).getValue();
            ...
            if (algorithm.equals((ASN1Primitive) PKCSObjectIdentifiers.dhKeyAgreement) || isPKCSParam(aSN1Sequence)) {
                ...
                if (validationParams != null) {
                    this.dhPublicKey = new DHPublicKeyParameters(this.f1563y, new DHParameters(domainParameters.getP(), domainParameters.getG(), domainParameters.getQ(), domainParameters.getJ(), new DHValidationParameters(validationParams.getSeed(), validationParams.getPgenCounter().intValue())));
                } else {
                    this.dhPublicKey = new DHPublicKeyParameters(this.f1563y, new DHParameters(domainParameters.getP(), domainParameters.getG(), domainParameters.getQ(), domainParameters.getJ(), (DHValidationParameters) null));
                }
    ...
    BCDHPublicKey(DHPublicKeyParameters dHPublicKeyParameters) {
        this.f1563y = dHPublicKeyParameters.getY();
    ...
    public byte[] getEncoded() {
        ...
        if (!(dHParameterSpec instanceof DHDomainParameterSpec) || ((DHDomainParameterSpec) dHParameterSpec).getQ() == null) {
            return KeyUtil.getEncodedSubjectPublicKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement, new DHParameter(this.dhSpec.getP(), this.dhSpec.getG(), this.dhSpec.getL()).toASN1Primitive()), new ASN1Integer(this.f1563y));
        }
    ...
    public String toString() {
        return DHUtil.publicKeyToString("DH", this.f1563y, new DHParameters(this.dhSpec.getP(), this.dhSpec.getG()));
    }
}
