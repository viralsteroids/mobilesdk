...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f295mX;

    /* renamed from: mY */
    private float[] f296mY;
    ...
    private void initPath(Path path) {
        ...
        this.f295mX = new float[iMin];
        this.f296mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f295mX[i2] = fArr[0];
            this.f296mY[i2] = fArr[1];
        }
        if (Math.abs(this.f295mX[0]) <= 1.0E-5d && Math.abs(this.f296mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f295mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f296mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f295mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        int i6 = iMin - 1;
        throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f295mX[0] + "," + this.f296mY[0] + " end:" + this.f295mX[i6] + "," + this.f296mY[i6]);
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f295mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f295mX[i2]) {
        ...
        float[] fArr = this.f295mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f296mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f296mY;
        float f6 = fArr2[i];
