...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f234mX;

    /* renamed from: mY */
    private final float[] f235mY;
    ...
    PathInterpolatorApi14(Path path) {
        ...
        int i2 = i + 1;
        this.f234mX = new float[i2];
        this.f235mY = new float[i2];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < i2; i3++) {
            pathMeasure.getPosTan((i3 * length) / i, fArr, null);
            this.f234mX[i3] = fArr[0];
            this.f235mY[i3] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f234mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f234mX[i2]) {
        ...
        float[] fArr = this.f234mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f235mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f235mY;
        float f6 = fArr2[i];
