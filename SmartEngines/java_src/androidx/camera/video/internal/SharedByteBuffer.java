...
public final class SharedByteBuffer implements Closeable {
    ...
    SharedByteBuffer share() {
        ...
        if (Logger.isDebugEnabled(TAG)) {
            ...
            Logger.m122d(TAG, String.format(Locale.US, "Ref count incremented: %d [%s]", Integer.valueOf(iIncrementAndGet), toString()));
        }
    ...
    protected void finalize() throws Throwable {
        try {
            if (closeInternal()) {
                Logger.m128w(TAG, String.format(Locale.US, "SharedByteBuffer closed by finalizer, but should have been closed manually with SharedByteBuffer.close() [%s]", toString()));
            }
        } finally {
            super.finalize();
        }
    ...
    private boolean closeInternal() {
        synchronized (this.mCloseLock) {
            ...
            if (Logger.isDebugEnabled(TAG)) {
                ...
                Logger.m122d(TAG, String.format(Locale.US, "Ref count decremented: %d [%s]", Integer.valueOf(iDecrementAndGet), toString()));
            }
            if (iDecrementAndGet == 0) {
                if (Logger.isDebugEnabled(TAG)) {
                    Logger.m122d(TAG, String.format(Locale.US, "Final reference released. Running final close action. [%s]", toString()));
                }
                try {
                    ...
                    Logger.m125e(TAG, String.format(Locale.US, "Unable to execute final close action. [%s]", toString()), e);
                }
            }
            return true;
        }
