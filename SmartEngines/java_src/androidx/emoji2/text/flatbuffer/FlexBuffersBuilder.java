...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    private final ReadWriteBuf f274bb;
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i3 = value2.key;
                do {
                    b = FlexBuffersBuilder.this.f274bb.get(i2);
                    b2 = FlexBuffersBuilder.this.f274bb.get(i3);
                    if (b == 0) {
                        return b - b2;
                    }
                    i2++;
    ...
    private int putKey(String str) {
        ...
        int iWritePosition = this.f274bb.writePosition();
        if ((this.flags & 1) != 0) {
            Integer num = this.keyPool.get(str);
            if (num == null) {
                byte[] bytes = str.getBytes(StandardCharsets.UTF_8);
                this.f274bb.put(bytes, 0, bytes.length);
                this.f274bb.put((byte) 0);
        ...
        this.f274bb.put(bytes2, 0, bytes2.length);
        this.f274bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(iWritePosition));
        return iWritePosition;
    }
    ...
    private Value writeBlob(int i, byte[] bArr, int i2, boolean z) {
        ...
        int iWritePosition = this.f274bb.writePosition();
        this.f274bb.put(bArr, 0, bArr.length);
        if (z) {
            this.f274bb.put((byte) 0);
        }
    ...
    private int align(int i) {
        int i2 = 1 << i;
        int iPaddingBytes = Value.paddingBytes(this.f274bb.writePosition(), i2);
        while (true) {
    ...
    private void writeInt(long j, int i) {
        if (i == 1) {
            this.f274bb.put((byte) j);
            return;
        }
        if (i == 2) {
            this.f274bb.putShort((short) j);
        } else if (i == 4) {
            this.f274bb.putInt((int) j);
        } else {
            if (i != 8) {
                return;
            }
            this.f274bb.putLong(j);
        }
    }
    ...
    public ByteBuffer finish() {
        int iAlign = align(this.stack.get(0).elemWidth(this.f274bb.writePosition(), 0));
        writeAny(this.stack.get(0), iAlign);
        this.f274bb.put(this.stack.get(0).storedPackedType());
        this.f274bb.put((byte) iAlign);
        this.finished = true;
        return ByteBuffer.wrap(this.f274bb.data(), 0, this.f274bb.writePosition());
    }
    ...
    private Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        if (value != null) {
            iMax = Math.max(iMax, value.elemWidth(this.f274bb.writePosition(), 0));
            i4 = 3;
        } else {
            i4 = 1;
        }
        ...
        for (int i7 = i2; i7 < this.stack.size(); i7++) {
            iMax2 = Math.max(iMax2, this.stack.get(i7).elemWidth(this.f274bb.writePosition(), i7 + i4));
            if (z && i7 == i2) {
        ...
        if (!z) {
            while (i8 < this.stack.size()) {
                this.f274bb.put(this.stack.get(i8).storedPackedType(iMax2));
                i8++;
            }
        }
        if (value != null) {
    ...
    private void writeOffset(long j, int i) {
        writeInt((int) (this.f274bb.writePosition() - j), i);
    }
    ...
    private void writeDouble(double d, int i) {
        if (i == 4) {
            this.f274bb.putFloat((float) d);
        } else if (i == 8) {
            this.f274bb.putDouble(d);
        }
    }
    ...
    private Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            int i4 = i3 + 1;
            iMax = Math.max(iMax, Value.elemWidth(4, 0, this.stack.get(i3).key, this.f274bb.writePosition(), i4));
            i3 = i4;
        }
