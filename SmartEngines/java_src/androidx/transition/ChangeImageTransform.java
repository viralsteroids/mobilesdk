...
public class ChangeImageTransform extends Transition {
    ...
    private void captureValues(TransitionValues transitionValues, boolean z) {
        ...
        if ((view instanceof ImageView) && view.getVisibility() == 0) {
            ...
            Matrix matrixCopyImageMatrix = z ? (Matrix) imageView.getTag(C2617R.id.transition_image_transform) : null;
    ...
    private static Matrix copyImageMatrix(ImageView imageView) {
        ...
        if (drawable.getIntrinsicWidth() > 0 && drawable.getIntrinsicHeight() > 0) {
            int i = C26073.$SwitchMap$android$widget$ImageView$ScaleType[imageView.getScaleType().ordinal()];
            if (i == 1) {
    ...
    private static class Listener extends AnimatorListenerAdapter implements Transition.TransitionListener {
        ...
        private void restoreMatrix() {
            Matrix matrix = (Matrix) this.mImageView.getTag(C2617R.id.transition_image_transform);
            if (matrix != null) {
                ImageViewUtils.animateTransform(this.mImageView, matrix);
                this.mImageView.setTag(C2617R.id.transition_image_transform, null);
            }
        }

        private void saveMatrix(Matrix matrix) {
            this.mImageView.setTag(C2617R.id.transition_image_transform, matrix);
