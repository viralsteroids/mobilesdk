...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        try {
            ...
            objM7850constructorimpl = Result.m7850constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
        } catch (Throwable th) {
        ...
        return (Function1) objM7850constructorimpl;
    }
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        if (Intrinsics.areEqual(state, DebugCoroutineInfoImplKt.RUNNING) && thread != null) {
            try {
                Result.Companion companion = Result.INSTANCE;
                DebugProbesImpl debugProbesImpl = this;
                objM7850constructorimpl = Result.m7850constructorimpl(thread.getStackTrace());
            } catch (Throwable th) {
                Result.Companion companion2 = Result.INSTANCE;
                objM7850constructorimpl = Result.m7850constructorimpl(ResultKt.createFailure(th));
            }
            ...
            StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM7850constructorimpl;
    ...
    private final Pair<Integer, Integer> findContinuationStartIndex(int indexOfResumeWith, StackTraceElement[] actualTrace, List<StackTraceElement> coroutineTrace) {
        for (int i = 0; i < 3; i++) {
            ...
            if (iFindIndexOfFrame != -1) {
                return TuplesKt.m521to(Integer.valueOf(iFindIndexOfFrame), Integer.valueOf(i));
            }
        }
        return TuplesKt.m521to(-1, 0);
    }
