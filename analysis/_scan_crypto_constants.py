...
REPORT = os.path.join(ROOT, 'analysis', 'crypto_constants_scan.md')
...
PATTERNS = [
    ...
    ('sha256_k_le', words_to_le_bytes(SHA256_K)),
    ('blake2s_iv_le', words_to_le_bytes(BLAKE2S_IV)),
    ('chacha_const_32', b'expand 32-byte k'),
    ('chacha_const_16', b'expand 16-byte k'),
]
...
for name, pat in PATTERNS:
    ...
    while True:
        ...
        if idx == -1:
            break
        hits.append((name, idx, len(pat)))
        start = idx + 1

with open(REPORT, 'w', encoding='ascii') as out:
    out.write('# Crypto constant scan (libjnimultiengine.so)\n\n')
    out.write(f'File: {LIB}\n\n')
    if not hits:
        out.write('No direct matches for AES S-box, AES inv S-box, SHA-256 K, BLAKE2s IV, or ChaCha constants.\n')
    else:
