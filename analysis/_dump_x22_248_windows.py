...
INP = Path(r"C:\temp\MobileSDK\analysis\x22_248_ref_scan.md")
OUT = Path(r"C:\temp\MobileSDK\analysis\x22_248_ref_windows.txt")
...
with LIB.open('rb') as f:
    elf = ELFFile(f)
    lines = ["# LDR [x22+0x248] windows", "", f"Library: {LIB}", f"Window: +/- 0x{WINDOW:x}", ""]
    for addr in addrs:
