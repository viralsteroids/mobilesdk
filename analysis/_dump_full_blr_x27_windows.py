...
blr_hits = [0x863990, 0x863e24, 0x863ffc, 0x1af1bc0, 0x1af2864, 0x22aeadc]
...
for addr in blr_hits:
    out_lines.append("")
    out_lines.append(f"BLR X27 window around 0x{addr:x}")
    for i in range(-WINDOW, WINDOW+1):
        ...
        tag = "<BLR>" if a == addr else ""
        out_lines.append(f"  0x{a:x}: {insn:08x} {dec} {tag}")

out_path = Path(r"C:\temp\MobileSDK\analysis\full_blr_x27_windows.txt")
