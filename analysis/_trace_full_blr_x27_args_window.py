...
callsites = [0x863990, 0x863e24, 0x863ffc]
window = 0x200
...
def trace_window(callsite):
    ...
    lines = []
    lines.append(f"Callsite 0x{callsite:x} window 0x{start:x}..0x{callsite:x}")
    lines.append(f"SP offset at callsite (relative SP0) = 0x{sp_off:x}")
    for r in [0,1,19,24,25,26,27]:
        lines.append(f"X{r} = {expr.get(r)}")
    lines.append("Stack slots (0x0..0x120):")
    for s in sorted(k for k in stack.keys() if 0x0 <= k <= 0x120):
        lines.append(f"  SP+0x{s:x} = {stack[s]}")
    return lines

out_lines = []
for cs in callsites:
    out_lines.extend(trace_window(cs))
    out_lines.append("")

out = Path(r"C:\temp\MobileSDK\analysis\full_blr_x27_arg_trace.md")
