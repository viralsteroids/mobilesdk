...
def decode_add_imm(insn):
    if (insn & 0x7F000000) == 0x91000000:  # ADD Xd, Xn, #imm
        rd = insn & 0x1F
        rn = (insn >> 5) & 0x1F
        imm12 = (insn >> 10) & 0xFFF
        shift = (insn >> 22) & 0x1
...
def analyze_callsite(call_addr, window=80):
    start = call_addr - window * 4
    if start < text_addr:
        start = text_addr
    end = call_addr
    # track expressions for X0-X30
    expr = {i: f"X{i}" for i in range(31)}
    imm = {i: None for i in range(31)}
    imm_mask = {i: 0 for i in range(31)}
    last_set = {i: None for i in range(31)}  # addr of last set
...
for target in focus:
    ...
    for caddr in calls:
        info = analyze_callsite(caddr, window=120)
        parts = []
        for r in range(5):
            expr, last = info[r]
            if last is not None:
...
out_path = Path(r"C:\temp\MobileSDK\analysis\apktool_zstd_dict_callsite_arg_scan_v2.md")
