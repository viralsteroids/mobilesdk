...
for addr in TARGETS:
    ...
    if func is None:
        print('no function at', hex(addr)); continue
    ...
    if not res.decompileCompleted() or res.getDecompiledFunction() is None:
        print('decompile failed', hex(addr)); continue
    out=OUT_DIR/ f"from_embedded_caller_{addr:08x}.c"
