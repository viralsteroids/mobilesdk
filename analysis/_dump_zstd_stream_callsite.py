...
def decode_add_imm(insn):
    # ADD (immediate, 64-bit) opcode 0x91xxxxxx
...
ZSTD_CALL = 0x027670b0
WINDOW = 12

lines = []
lines.append("# ZSTD_decompressStream callsites (decoded windows)")
...
for p_offset, p_vaddr, p_filesz, p_flags in segs:
    ...
    for i in range(0, len(seg) - 4, 4):
        ...
        if target != ZSTD_CALL:
            continue
        lines.append("")
        lines.append(f"Callsite @0x{pc:x} -> ZSTD_decompressStream")
        # back
...
Path(r"C:\temp\MobileSDK\analysis\zstd_decompress_stream_callsites.md").write_text(
    "\n".join(lines), encoding="ascii", errors="ignore"
)
print(r"C:\temp\MobileSDK\analysis\zstd_decompress_stream_callsites.md")
