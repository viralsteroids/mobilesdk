...
# typedef struct {
#   uint32 sh_name; uint32 sh_type; uint64 sh_flags; uint64 sh_addr; uint64 sh_offset; uint64 sh_size;
#   uint32 sh_link; uint32 sh_info; uint64 sh_addralign; uint64 sh_entsize;
# }
...
# Callers from decoder_chain_callers.md
callers = [0x0b86ca8, 0x01190020, 0x011e6750, 0x0126c3ec]

# Map each caller to nearest symbol <= address
lines = []
lines.append('# Caller symbol mapping (dynsym)')
lines.append('')
for addr in callers:
    # binary search
    lo = 0
...
Path(r"C:\temp\MobileSDK\analysis\decoder_chain_callers_dynsym.md").write_text("\n".join(lines), encoding='ascii', errors='ignore')
print(r"C:\temp\MobileSDK\analysis\decoder_chain_callers_dynsym.md")
