...
OUT = Path(r"C:\temp\MobileSDK\analysis\x22_248_store_scan.md")
...
with LIB.open('rb') as f:
    ...
    for i in range(0, len(data), 4):
        ...
        di = decode_str_imm(word)
        if not di:
            continue
        rn, rt, off = di
        if rn == REG and off == OFFSET:
            hits.append((base + i, rt))

    out = [
        f"# STR [{reg_name(REG)}+0x{OFFSET:x}] reference scan", "",
    ...
    if not hits:
        ...
        for addr, rt in hits:
            out.append(f"- 0x{addr:08x} : str {reg_name(rt)}, [x22+0x248]")
